...
export default function CalendarScreen() {
  ...
  const resolvedSelected = selectedDateKey || (() => {
    const first = Object.keys(events).find(key => {
  ...
  return (
    <>
      ...
      <div style={{ padding: '2px 14px 6px', fontSize: '13px', fontWeight: 500, color: 'var(--c-t2)', display: 'flex', alignItems: 'center', gap: '6px' }}>
        {year}년 {month + 1}월
        {hasEvents && <span className="ghost-badge">예시</span>}
      </div>
      <div className="scroll-area">
        <div style={{ padding: '0 14px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: '6px', opacity: hasEvents ? 0.6 : 1 }}>
            {cells}
          </div>
        </div>

        <div style={{ padding: '14px 14px 6px', fontSize: '12px', fontWeight: 600, color: 'var(--c-t3)', letterSpacing: '.5px' }}>
          {selYear}년 {Number(selMonth)}월 {Number(selDay)}일 일정
        </div>
        <div style={{ padding: '0 14px 20px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {selectedEvents.length === 0 ? (
            ...
            selectedEvents.map((event, i) => (
              <div key={i} style={{ display: 'flex', gap: '10px', padding: '12px 13px', border: '1.5px solid var(--c-border)', borderRadius: '13px', background: 'var(--c-surface)', opacity: 0.6 }}
                onClick={() => showToast('이 일정은 예시입니다')}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '6px' }}>
                    <div style={{ fontSize: '10px', fontWeight: 500, padding: '2px 8px', borderRadius: '8px', background: '#F5F4F0', color: 'var(--c-t2)' }}>
                      {event.type}
                    </div>
