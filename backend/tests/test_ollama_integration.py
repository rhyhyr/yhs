...
# ── src 레이아웃이므로 파일 경로 로드 없이 그냥 임포트한다.
#    환경변수(LLM_PROVIDER 등)를 바꿔 가며 검증하는 테스트가 있어
#    모듈 캐시를 비우고 재임포트하는 헬퍼를 둔다.
# 환경변수를 import 시점에 모듈 상수로 굳히는 모듈들.
# LLM_PROVIDER/GEMINI_API_KEY 를 바꿔 가며 검증하려면 캐시를 비워야 한다.
_ENV_BOUND_MODULES = (
    "yhs.ingest.pipeline.extractor",
    "yhs.ingest.llm.gemini_client",
    "yhs.ingest.llm.ollama_kb_client",
    "yhs.ingest.llm.openai_client",
    "yhs.ingest.llm.exaone_kb_client",
    "yhs.core.config",
    # config 의 상수는 get_settings() 에서 오는데 그게 lru_cache 다.
    # 이 모듈까지 버려야 캐시가 같이 사라지고 monkeypatch 한 환경변수가 읽힌다.
    # 빠져 있던 탓에 Gemini 라우팅 테스트가 .env 있는 로컬에서만 통과했다.
    "yhs.core.settings",
)


def _load_extractor():
    for _m in _ENV_BOUND_MODULES:
        sys.modules.pop(_m, None)
    return importlib.import_module("yhs.ingest.pipeline.extractor")


def _load_ollama_runtime():
    return importlib.import_module("yhs.rag.llm.ollama_client")
